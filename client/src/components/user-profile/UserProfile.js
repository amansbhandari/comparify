...
const style = {
  ...
  imageContainer: {
    ...
    alignItems: "center",
    background:
      "linear-gradient(45deg, rgb(17 69 147) 0%, rgb(44 87 158) 35%, rgb(73 109 171) 100%);",
  },
  image: {
    width: "100%",
  },
...
const UserProfile = (props) => {
  ...
  useEffect(() => {
    getDetails(localStorage.getItem("user-id"))
      .then((data) => {
        ...
        if (isDataLoaded === false) setDataLoaded(true);
      })
  ...
  function editClicked() {
    if (isEditMode === false) {
      //Click to edit
      setEditMode(true);
      if (isDataLoaded === true) setDataLoaded(false);
  ...
  function showEmailId() {
    if (isEditMode) {
      ...
      return (
        <label style={style.editableField} disabled={isEditMode}>
          {userEmail()}
        </label>
      );
    }
